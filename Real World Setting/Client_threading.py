...
class Data_Receiving_Thread(threading.Thread):
    ...
    def make_predictions(self, prediction_weights, dataset, label, pred_time):
        ...
        logging.info(f'Client {CLIENT_ID}: False positives: {fp}, False negatives: {fn}, True positives: {tp}, True negatives: {tn}, F1: {f1}')
    ...
    while 1:
        ...
        if registered:
            ...
            if (len(data_thread.train_data_batch) >= 1):  #registered == True and
                ...
                train_dataset =  np.array([item for sublist in data_thread.train_data_batch for item in sublist]) # np.array(data_thread.train_data_batch.pop(0))

                # train_labels = [item for sublist in data_thread.train_data_label for item in sublist] #np.array(data_thread.train_data_label.pop(0))
            ...
            print(f"Client {CLIENT_ID}: Collected: Training: ",
                  ...
                  "New Batch: [" + '█' * int(len(data_thread.data_container)) + ' ' * int((DATA_STEPSIZE - int(
                      len(data_thread.data_container)))) + "]")  # len(newthread.data_container))
