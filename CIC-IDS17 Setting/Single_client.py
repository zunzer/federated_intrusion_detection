...
# Single Client which is not sending
# any model related information to the server.
# This allows us to simulate a traditional setup
# that is trained without federated learning.
...
class Data_Receiving_Thread(threading.Thread):
    ...
    def start_prediction(self):
        # if enough training rounds made
        if train_count > 0:
            ...
            self.make_predictions(LOCAL_WEIGHTS, pred_dataset, pred_labels, pred_time)
    ...
    def make_predictions(self, prediction_weights, dataset, label, pred_time):
        ...
        for i in range(0, len(label)):
            if not label[i][0] == "BENIGN":
                ...
                # if outliers[i] == True:
                #    with open(f'results/times_{label[i]}.txt', "a+", newline='') as f:
...
def store_loss(loss_hist):
    ...
    if CLIENT_ID == 1:
        with open('results/loss_1.txt', 'a') as file:
...
if __name__ == "__main__":
    ...
    enc, dec = end.build()
    local_model = tf.keras.models.Model(inputs=input_format, outputs=dec(enc(input_format)))
    ...
    while 1:
        ...
        if registered:
            try:
                ...
                scraped_weights = recv_global_weights_from_server(s)
            ...
            if (len(data_thread.train_data_batch) > 1):  # registered == True and
                ...
                # train local model
                LOCAL_WEIGHTS = client_update(LOCAL_WEIGHTS, train_dataset)
                local_model.set_weights(LOCAL_WEIGHTS)  # set local model weight to the weight of the global model
                ...
                try:
                    ...
                    send_weights_to_server(LOCAL_WEIGHTS, data_thread.metrics)
